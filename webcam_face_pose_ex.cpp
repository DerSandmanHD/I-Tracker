...
void calculation(int p_r_edge_right_x, int p_r_edge_left_x, int p_l_edge_right_x, int p_l_edge_left_x, int p_r_edge_right_y, int p_r_edge_left_y, int p_l_edge_right_y, int p_l_edge_left_y, int nose_x, int nose_y) {

	

	// float iris_distance = hypot(p_r.x - p_l.x, p_r.y -p_l.y);

	iris_distance = hypot(((p_r_edge_right_x + p_r_edge_left_x) / 2 - (p_l_edge_right_x + p_l_edge_left_x) / 2), ((p_r_edge_right_y + p_r_edge_left_y) / 2 - (p_l_edge_right_y + p_l_edge_left_y) / 2));

	// Berechnung der Distanz von (Rechtes)Auge zu Nase

	right_eye_nose_distance = hypot((p_r_edge_right_x + p_r_edge_left_x) / 2 - nose_x, (p_r_edge_right_y + p_r_edge_left_y) / 2 - nose_y);

	// Berechnung der Distanz von (Linkes)Auge zu Nase

	left_eye_nose_distance = hypot((p_l_edge_right_x + p_l_edge_left_x) / 2 - nose_x, (p_l_edge_right_y + p_l_edge_left_y) / 2 - nose_y);
...

{
...

    {
		...
		//initialisieren von Kalibration Arrays 
		...
		// initialisieren von Button Arrays (speichern Positionen der Buttons)
...

        {
			...
			for (unsigned long i = 0; i < faces.size(); ++i) {
				...
				int nose_x = shape.part(34).x();

				int nose_y = shape.part(34).y();

				// Landmark des rechten Auge -> rechte Ecke

				int p_r_edge_right_x = shape.part(46).x();

				int p_r_edge_right_y = shape.part(46).y();



				// Landmark des rechten Auge -> linke Ecke

				int p_r_edge_left_x = shape.part(43).x();

				int p_r_edge_left_y = shape.part(43).y();

				// Landmark des linkes Auge -> rechte Ecke

				int p_l_edge_right_x = shape.part(40).x();

				int p_l_edge_right_y = shape.part(40).y();

				// Landmark des linkes Auge -> linke Ecke

				int p_l_edge_left_x = shape.part(37).x();

				int p_l_edge_left_y = shape.part(37).y();
				...
				//Wenn Button gedr�ckt wird, dann Distanzen berechnen und speichern
					...
					if (My_screen.buttonIsPressed1 == true) {
						...
						calib_array_1[0] = iris_distance ;

						calib_array_1[1] = right_eye_nose_distance;

						calib_array_1[2] = left_eye_nose_distance;

						calib_array_1[3] = p_r.x;

						calib_array_1[4] = p_r.y;

						calib_array_1[5] = p_l.x;

						calib_array_1[6] = p_l.y;
					...
					if (My_screen.buttonIsPressed2 == true) {
						...
						calib_array_2[0] = iris_distance;

						calib_array_2[1] = right_eye_nose_distance;

						calib_array_2[2] = left_eye_nose_distance;

						calib_array_2[3] = p_r.x;

						calib_array_2[4] = p_r.y;

						calib_array_2[5] = p_l.x;

						calib_array_2[6] = p_l.y;
					...
					if (My_screen.buttonIsPressed3 == true) {
						...
						calib_array_3[0] = iris_distance;

						calib_array_3[1] = right_eye_nose_distance;

						calib_array_3[2] = left_eye_nose_distance;

						calib_array_3[3] = p_r.x;

						calib_array_3[4] = p_r.y;

						calib_array_3[5] = p_l.x;

						calib_array_3[6] = p_l.y;
					...
					if (My_screen.buttonIsPressed4 == true) {
						...
						calib_array_4[0] = iris_distance;

						calib_array_4[1] = right_eye_nose_distance;

						calib_array_4[2] = left_eye_nose_distance;

						calib_array_4[3] = p_r.x;

						calib_array_4[4] = p_r.y;

						calib_array_4[5] = p_l.x;

						calib_array_4[6] = p_l.y;
					...
					if (My_screen.buttonIsPressed5 == true) {
						...
						calib_array_5[0] = iris_distance;

						calib_array_5[1] = right_eye_nose_distance;

						calib_array_5[2] = left_eye_nose_distance;

						calib_array_5[3] = p_r.x;

						calib_array_5[4] = p_r.y;

						calib_array_5[5] = p_l.x;

						calib_array_5[6] = p_l.y;
